...
const poll = {
  ...
  registerNewAnswer: function () {
    let questionAndAnswers = this.question;
    for (let i = 0; i < this.options.length; i++) {
      questionAndAnswers = questionAndAnswers + '\n' + this.options[i];
    }
    questionAndAnswers = questionAndAnswers + '\n(Write option number)';
    let answer = Number(prompt(questionAndAnswers));
  ...
  displayResults: function (type = 'array') {
    if (type === 'array') {
      ...
      let message = this.answers[0];
      for (let i = 1; i < this.answers.length; i++) {
        message = message + ', ' + this.answers[i];
      }
      console.log(`Poll results are ${message}`);
