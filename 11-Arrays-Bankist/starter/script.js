...
// const dogSarah = dogs.find(dog => dog.owners.find(owner => owner === 'Sarah'));
// console.log(dogSarah);

const dogSarah = dogs.find(dog => dog.owners.includes('Sarah'));
if (dogSarah.curFood < dogSarah.recomendedFood) {
  console.log('Too little');
} else if (dogSarah.curFood > dogSarah.recomendedFood) {
...
// 5. Log to the console whether there is any dog eating EXACTLY the amount of food that is recommended (just true or false)
// const perfDog = dogs.find(dog => dog.curFood === dog.recomendedFood);
// console.log(perfDog !== undefined);
// console.log(!!perfDog);
console.log(!!dogs.find(dog => dog.curFood === dog.recomendedFood));
console.log(
  dogs.filter(dog => dog.curFood === dog.recomendedFood).length !== 0
);
...
console.log(dogsEatingOK.length !== 0);
...
// console.log(arr);
dogs.concat([]);
dogs.filter(dog => true);
console.log(dogs.map(dog => dog));
dogs.map(function (x, i, arr) {
  return x;
});
const arr = [];
dogs.forEach(function (dog) {
...
const sortedDogs = dogs.map(dog => dog);
sortedDogs.sort(function (dog1, dog2) {
  if (dog1.recomendedFood > dog2.recomendedFood) return 1;
  if (dog1.recomendedFood < dog2.recomendedFood) return -1;
  if (dog1.recomendedFood === dog2.recomendedFood) return 0;
});
console.log(sortedDogs);

sortedDogs.sort((dog1, dog2) => dog1.recomendedFood - dog2.recomendedFood);
...
// 5. some -will return true if at least one satisfy the condition
